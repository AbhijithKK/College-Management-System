...
let faculty = {
    ...
    try {
      ...
      if (req.query.search) {
        let key = req.query.search
          .replace(/[^a-bA-B]/g, "")
          .replace(/[^a-bA-B]/g, "");
        allRequests = await clubRequestScheema
          .find({ clubAdminId: verify.data, studentName: new RegExp(key, "i") })
          .sort({ _id: -1 })
          .exec();
      } else {
        allRequests = await clubRequestScheema
          .find({ clubAdminId: verify.data })
          .sort({ _id: -1 })
          .exec();
      }
    ...
    try {
      ...
      if (req.query.search) {
        let key = req.query.search
          .replace(/[^a-bA-B]/g, "")
          .replace(/[^a-bA-B]/g, "");
        allLeaveletters = await leaveApplyScheema
          .find({ studentName: new RegExp(key, "i") })
          .sort({ _id: -1 })
          .exec();
      } else {
        allLeaveletters = await leaveApplyScheema
  ...
  PreviousAttendance: async (req, res) => {
    try {
      ...
      let verify = await jwtVerify(req.cookies.facultyjwt);
      let data = await facultyModel.findOne({ _id: verify.data });
      let attendance = await attendenceScheema
        .find({
          $and: [
            { facultyId: data._id },
            { className: data.adminOfClass },
            { studentName: new RegExp(key, "i") },
          ],
          date: new RegExp(datekey, "i"),
        })
        .sort({ _id: -1 })
        .exec();
      res.json(attendance);
  ...
  getAttendance: async (req, res) => {
    try {
      ...
      let temp = await attendenceScheema
        .find({
          $and: [
            { date: todayDate },
            { facultyId: data._id },
  ...
  getAdminClubs: async (req, res) => {
    try {
      ...
      let clubs = await club
        .find({ clubAdminId: verify.data, name: new RegExp(key) })
        .sort({ _id: -1 })
        .exec();
      res.json(clubs);
    } catch (err) {
  ...
  postMailVerify: async (req, res) => {
    try {
      let data = await facultyModel.findOne({
        $or: [{ email: req.body.data }, { mobNumber: req.body.data }],
      });
      ...
      if (data !== null) {
        ...
        res.json({
          otp: false,
          text: "Enter Your Registerd Email or Mobile Number",
        });
      }
    } catch (err) {
      res.json(false);
  ...
  clubRequestUpdate: async (req, res) => {
    try {
      await clubRequestScheema.updateOne(
        { _id: req.body.id },
        { status: req.body.status }
      );
      res.json(true);
  ...
  LeaveStatusUpdate: async (req, res) => {
    try {
      await leaveApplyScheema.updateOne(
        { _id: req.body.id },
        { status: req.body.status, adminName: req.body.adminName }
      );
  ...
  postAttendance: async (req, res) => {
    try {
      ...
      if (temp.length == 0) {
        ...
        let update = await attendenceScheema.updateOne(
          { _id: temp[0]._id },
          {
            date: req.body.details.date,
  ...
  applyPassword: async (req, res) => {
    try {
      ...
      let data = await facultyModel.updateOne(
        { email: req.body.email },
        { password: newPassword }
      );
  ...
  deleteMeeting: async (req, res) => {
    try {
      await club.findByIdAndUpdate(req.query.id, {
        $pull: { meeting: { id: req.query.meetingId } },
      });
      res.json(true);
    } catch (err) {
      res.json(false);
  ...
  GetClubStudents: async (req, res) => {
    try {
      let data = await clubRequestScheema
        .find({
          $and: [{ clubId: req.query.id }, { status: "Now Your a Member" }],
        })
        .sort({ studentName: 1 })
        .lean();
      res.json(data);
