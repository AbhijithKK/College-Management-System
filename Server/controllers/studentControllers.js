...
let student = {
  ...
  getAttendance: async (req, res) => {
    try {
      let verify = await jwtVerify(req.cookies.studentjwt);
      let data = await attendenceScheema
        .find({ studentId: verify.data })
        .sort({ _id: -1 })
        .exec();
      res.json(data);
    } catch (err) {
  ...
  getResult: async (req, res) => {
    try {
      ...
      let data = await resultScheema
        .find({ studentId: verify.data, semester: new RegExp(key, "i") })
        .sort({ _id: -1 })
        .exec();
      res.json(data);
    } catch (err) {
  ...
  getLeaveletters: async (req, res) => {
    try {
      ...
      let allLeaveletters = await leaveApplyScheema
        .find()
        .sort({ _id: -1 })
        .exec();
      ...
      if (history) {
        for (let i = 0; i < pay.length; i++) {
          ...
          for (let j = 0; j < history.length; j++) {
            if (
              pay[i]._id.equals(history[j].paymentId) &&
              history[j].studentId == verify.data
            ) {
              s = history[j].status;
            }
          }
  ...
  calander: async (req, res) => {
    try {
      ...
      let id = verify.data;
      let notification = [];
      let cluReq = await clubRequestScheema
        .find({ $and: [{ studentId: id }, { status: "Now Your a Member" }] })
        .lean();
      ...
      for (let i = 0; i < cluReq.length; i++) {
        ...
        for (let i = 0; i < notification.length; i++) {
          for (let j = 0; j < notification[i].meeting?.length; j++) {
            ...
            const [day, month, year] = notification[i].meeting[j].date
              .split("/")
              .map(Number);
            const adjustedHours =
              meridiem.toLowerCase() === "pm" ? hours + 12 : hours;
      ...
      if (payHistory) {
        for (let i = 0; i < payHistory.length; i++) {
          let color = "";
          let isoFormat = new Date(payHistory[i].date)
            .toISOString()
            .slice(0, -5);
          let currentDate = new Date().toISOString().slice(0, -5);
          if (currentDate < isoFormat) {
  ...
  PaymentSuccess: async (req, res) => {
    try {
      ...
      let data = await paymentHistoryModel.updateOne(
        { _id: req.query.id },
        { status: true }
      );
