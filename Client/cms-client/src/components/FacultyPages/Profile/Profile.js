...
//   }
  ...
  const HandlSave = () => {
    if (name.trim() && email.trim() && mobNumber.trim() && dob.trim() && admYear.trim() && qualifications.trim() && teachingArea.trim()
      && address.trim() && department.trim() && gender.trim() ) {

      FacultyProfileUpdateApi(name, email, mobNumber, dob, admYear, qualifications, teachingArea, address, department,
        gender,  pic)
      setRefresh(!refresh)
      setOpen(false);
    } else {
              ...
              <MDBCard className="mb-3" style={{ borderRadius: '.5rem' }}>
                <MDBRow className="g-0">
                  <MDBCol md="4" className="gradient-custom text-center text-black"
                    ...
                    <MDBCardImage src={FacultyData.image === 'noImg' ? 'https://mdbcdn.b-cdn.net/img/Photos/new-templates/bootstrap-chat/ava1-bg.webp' : `http://localhost:4000/images/${FacultyData.image}`}
                    ...
                    <MDBCardBody className="p-4">
                      ...
                      <MDBRow className="pt-1">
                        <MDBCol size="6" className="mb-3">
                          ...
                          <MDBCardText className="text-muted">{FacultyData.email}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">Phone</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.mobNumber}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">D-O-B</MDBTypography>
                          <MDBCardText className="text-muted">{new Date(FacultyData.DOB).toLocaleDateString()}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">Gender</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.gender}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">Admission Year</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.admYear}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">Department</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.department}</MDBCardText>
                        </MDBCol>
                      ...
                      <MDBRow className="pt-1">
                        <MDBCol size="6" className="mb-3">
                          ...
                          <MDBCardText className="text-muted">{FacultyData.qualifications}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">Guardian Number</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.teachingArea}</MDBCardText>
                        </MDBCol>
                        <MDBCol size="6" className="mb-3">
                          <MDBTypography tag="h6">address</MDBTypography>
                          <MDBCardText className="text-muted">{FacultyData.address}</MDBCardText>
                        </MDBCol>
                      </MDBRow>
