...
export default function Clubs() {
  ...
  const datafilter = useCallback(() => {
    const arr = clubs?.map((club) => {
      const clubStatus = status.find(
        (s) => s.studentId === studentData._id && s.clubId === club._id
      );
      return {
        clubId: club._id,
        clubName: club.name,
  ...
  return (
    <div style={{ backgroundColor: "gray", height: "100vh",overflow:"hidden" }}>
      ...
      <Container>
        {clubs?.length === 0 ? (
          <h1 className="clubReqTitle">No Clubs Created</h1>
        ) : (
          <h1 className="clubReqTitle">Your Clubs</h1>
        )}
                      ...
                      style={{color:"#ffff",
                        ...
                        backgroundColor:
                          data.clubStatus === "Request Send"
                            ? "orange"
                            : data.clubStatus === "Now Your a Member"
                            ? "green"
                            : "red",
                      }}
                    >
